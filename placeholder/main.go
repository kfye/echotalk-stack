...
// Day 2 起由真实 backend 仓替换本服务（暴露同样的 :8080 与 /health 即可无缝替换）。
package main
...
func main() {
	// 容器 healthcheck 入口：自检本地 /health，200 即退出 0。
	...
	// 永远返回 200（仅表示服务存活）；依赖状态放 body 供观察。
	...
	// 启动时打印一次依赖探测，方便在 Portainer 日志里确认链路。
	if b, _ := json.Marshal(checkDeps()); b != nil {
